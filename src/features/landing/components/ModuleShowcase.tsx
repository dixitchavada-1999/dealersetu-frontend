    ...
    <div id="modules" className="mt-12 relative max-w-6xl mx-auto">
        ...
        {modules.map((m, i) => {
          ...
          return (
            <button
              ...
              style={{
                background: isActive ? 'linear-gradient(135deg, #0F52BA 0%, #3B82F6 100%)' : '#ffffff',
                borderColor: isActive ? 'transparent' : '#e2e8f0',
                color: isActive ? '#fff' : '#475569',
                boxShadow: isActive ? '0 8px 20px rgba(15, 82, 186, 0.25)' : 'none',
              }}
            >
              <m.icon size={16} />
              {m.title}
      ...
      <div className="max-w-xs mx-auto mb-8 h-0.5 rounded-full overflow-hidden" style={{ background: 'rgba(15, 23, 42, 0.08)' }}>
        <div
          ...
          className="h-full"
          style={{ background: 'linear-gradient(90deg, #0F52BA, #3B82F6)', animation: isHovered ? 'none' : 'moduleProgress 4s linear', transformOrigin: 'left', width: '100%' }}
        />
      </div>

      <div className="relative" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <div className="absolute -inset-px rounded-3xl blur opacity-50" style={{ background: 'linear-gradient(135deg, #0F52BA, #3B82F6)' }} />
        <div className="relative rounded-3xl border backdrop-blur-xl shadow-2xl overflow-hidden" style={{ backgroundColor: '#ffffff', borderColor: '#e2e8f0' }}>
          <div className="px-5 py-3 flex items-center gap-2 border-b" style={{ backgroundColor: '#f1f5f9', borderColor: '#e2e8f0' }}>
            <div className="flex gap-1.5">
              <span className="w-3 h-3 rounded-full" style={{ background: '#ef4444' }} />
              <span className="w-3 h-3 rounded-full" style={{ background: '#eab308' }} />
              <span className="w-3 h-3 rounded-full" style={{ background: '#22c55e' }} />
            </div>
            <span className="text-xs ml-3 font-mono" style={{ color: '#475569' }}>app.dealersetu.com</span>
          </div>
